  ...
  return (
    <div className="my-2 rounded-sm border-2 border-neutral-200 hover:bg-neutral-100" id={mdBlock.blockId}>
      <Link href={data.url} target='_brank' rel="noopener noreferrer">
          <div className="flex">
              <div className='m-3'>
                  <p className='text-neutral-800 line-clamp-1'>{data.downloadUrl && ogData ? ogData.meta.title : new URL(data.url).hostname}</p>
                  {data.downloadUrl && ogData && <p className='text-neutral-500 text-sm line-clamp-2'>{ogData.meta.description}</p>}
                  <div className='flex mb-0 mt-2'>
                      {data.downloadUrl && ogData && ogData.links.icon &&
              ...
              {data.downloadUrl && ogData && ogData.links.thumbnail && (
