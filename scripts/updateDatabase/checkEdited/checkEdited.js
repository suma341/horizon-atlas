...
export const getCurrentData=async()=>{
    ...
    // return null;
    const newData = editTimeData.filter((item1)=>filtered.every((item2)=>item1.id !== item2.curriculum && item1.update))
    console.log(`新たなページ":${newData.length}個`)
    const editedData = editTimeData.filter((item1)=>filtered.some((item2)=>
        item1.Last_edited_time !== item2.last_edited && item1.id === item2.curriculum && item1.update
    ))
